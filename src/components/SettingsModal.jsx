...
export const SettingsModal = ({ isOpen, onClose, onSettingsUpdated, onOpenSqlGuide }) => {
  ...
  const [imgBbToggle, setImgBbToggle] = useState(true);
  const [capacityGb, setCapacityGb] = useState(5);
  const [newPassword, setNewPassword] = useState('');
  const [savedSuccess, setSavedSuccess] = useState(false);
  const [activeTab, setActiveTab] = useState('supabase'); // 'supabase', 'imgbb', 'capacity', 'security'
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSupabaseUrl(url);
      setSupabaseKey(key);
      setImgBbKey(getImgBbApiKey());
      setImgBbToggle(isImgBbEnabled());
  ...
  const handleSave = async (e) => {
    ...
    // Save ImgBB API Key & Toggle State
    saveImgBbApiKey(imgBbKey);
    setImgBbEnabled(imgBbToggle);
  ...
  const handleClearImgBb = () => {
    saveImgBbApiKey('');
    setImgBbKey('');
    setImgBbToggle(false);
  ...
  const tabList = [
    ...
    { id: 'imgbb', label: 'ImgBB Images', icon: Image },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-2xl animate-fade-in">
      <div className="relative w-full max-w-xl glass-modal-premium rounded-3xl p-6 sm:p-8 border border-white/15 shadow-2xl space-y-6 animate-modal-entrance">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400">Configure storage providers & security settings</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Tab 2: ImgBB Image CDN */}
        {activeTab === 'imgbb' && (
          <div className="space-y-4 animate-fade-in">
            <div className="p-3.5 rounded-2xl bg-zinc-900/90 border border-white/10 flex items-start gap-3 text-xs text-zinc-300 relative overflow-hidden gradient-border-card">
              <div className="w-1 absolute top-0 left-0 bottom-0 bg-pink-400" />
              <Image className="w-5 h-5 text-pink-400 flex-shrink-0 mt-0.5" />
              <div>
                <span className="font-bold text-white block mb-0.5">ImgBB Image CDN Option</span>
                Optionally enable ImgBB to store uploaded images (`.png`, `.jpg`, `.webp`, `.gif`) on ImgBB CDN to save Supabase storage space.
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-zinc-950/80 border border-white/10 flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-white block">Use ImgBB for Image Storage</span>
                <span className="text-[11px] text-zinc-400">Route all image uploads to ImgBB CDN automatically</span>
              </div>
              <button
                type="button"
                onClick={() => setImgBbToggle(!imgBbToggle)}
                className={`p-1 rounded-full transition-colors btn-press ${imgBbToggle ? 'text-pink-400' : 'text-zinc-600'}`}
              >
                {imgBbToggle ? <ToggleRight className="w-7 h-7" /> : <ToggleLeft className="w-7 h-7" />}
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="Enter your ImgBB 32-character API Key"
                  className="w-full bg-zinc-900 text-zinc-100 placeholder-zinc-600 text-xs rounded-xl px-3.5 py-2.5 border border-white/10 focus:outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 font-mono transition-all"
                />
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                <a
                  ...
                  className="text-xs text-pink-400 hover:text-pink-300 flex items-center gap-1 font-mono hover:underline btn-press"
                >
                  <ExternalLink className="w-3.5 h-3.5" />
                  <span>Get Free ImgBB API Key (api.imgbb.com)</span>
                </a>

                {isImgBbConfigured() && (
                  <button
                    type="button"
                    onClick={handleClearImgBb}
                    className="text-xs text-rose-400 hover:text-rose-300 btn-press font-semibold"
                  >
                    Disconnect ImgBB
                  </button>
